...
void draw()
{
    ...
    /*
        Here we first rotate the container around the origin (0,0,0) and once it's rotated, 
            we scale down and then we translate its rotated version to the bottom-right corner of the screen. 
        Remember that the actual transformation order should be read in reverse: even though in code 
            we first translate and scale and then later rotate, the actual transformations first apply a rotation and 
            then a scale and translation. 
    ...
    */
    ...
    /*
        ...
        The last parameter is the actual matrix data, but GLM stores their matrices' data in a way that doesn't always 
            match OpenGL's expectations so we first convert the data with GLM's built-in function value_ptr.
    */
    glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(trans));
    glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);

    float rotate = 0.5f * sin(glfwGetTime());
    if (rotate < 0)
    {
        rotate = -rotate;
    }
    
    glm::vec3 vec = glm::vec3(0.5f, 0.5f, 0.5f);
    glm::vec3 trans2 = glm::vec3(rotate, rotate, rotate);

    trans = glm::mat4(1.0f);
    trans = glm::translate(trans, glm::vec3(-0.5f, 0.5f, 0.0f));
    trans = glm::scale(trans, vec * trans2);
